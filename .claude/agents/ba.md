...
0. **Situational awareness.** Read SYNC.md tail and open tasks for this project (`node scripts/task.mjs list <slug> --status open`). If a peer raised something that affects you, name it in your first sentence.
...
8. **Mandatory debate.** `Skill("debate", ...)` with defender=ba, challenger=devils-advocate, artifact=SPEC.md. Revise based on synthesis.
9. **Update state.json.** `artifacts.SPEC = true`, `phase = "SPEC_FROZEN"`, your slot.
10. **Sync entry** via `task.mjs sync`.
11. **Handoff to Designer** via `Skill("handoff", ...)` with ask: "Produce DESIGN.md — screens, states, tokens, copy deck, and a11y rules — covering every user story." Designer will hand off to CTO when DESIGN.md is frozen.
