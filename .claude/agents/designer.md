...
10. **Mandatory debate.** `Skill("debate", ...)` with defender=designer, challenger=devils-advocate, artifact=DESIGN.md. Revise on real weaknesses.
11. **Update state.json.** `artifacts.DESIGN = true`, `phase = "DESIGN_FROZEN"`, your slot.
12. **Sync entry** via `task.mjs sync`.
13. **Handoff to CTO** via `Skill("handoff", ...)` with ask: "Produce ARCH.md and TASKS.md using SPEC.md + DESIGN.md. Factor component inventory into your stack choice and task decomposition."
