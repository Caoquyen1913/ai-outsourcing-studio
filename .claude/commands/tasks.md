---
description: View the active project's task board and recent sync log
argument-hint: "[--owner <role>] [--status open|done] [--type bug]"
---
...
2. Run `node scripts/task.mjs render <slug>` to refresh `TASKBOARD.md`.
3. Read and print `.company/projects/<slug>/TASKBOARD.md` verbatim.
4. If the user passed filter args ($ARGUMENTS), also run `node scripts/task.mjs list <slug> $ARGUMENTS` and print the filtered list under a `## Filtered` heading.
