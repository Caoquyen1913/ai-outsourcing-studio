...
- Check it contains all the required sections for its type (see `$HOME/.claude/ai-outsourcing-studio/references/CLAUDE.md` ownership table).
...
- Do NOT hand off to a role that does not own the next artifact in the pipeline (see `$HOME/.claude/ai-outsourcing-studio/references/CLAUDE.md` table).
