...
2. Produce that role's deliverable, update `.company/state.json`, run `node scripts/task.mjs sync <slug> --role <role> ...`.
3. In the same turn (or next), adopt the next role in the pipeline and repeat.
4. At every checkpoint, invoke the `debate` skill by loading `.claude/skills/debate/SKILL.md` and running the protocol with yourself as both defender and challenger — but label the outputs distinctly in the `REVIEWS/debate-<n>.md` file. It is less adversarial than Claude Code's real subagent debates, but still creates an audit trail.
...
# Update status
node scripts/task.mjs update <slug> <id> --status <open|in_progress|in_review|done|cancelled> --note "..."

# List / filter
node scripts/task.mjs list <slug> [--status open] [--owner dev] [--type bug]
...
Every bug QA finds MUST become a `B-NNN` task. See the **Bug Loop** section in `CLAUDE.md` for the full re-verify protocol.
