...
4. **Mandatory debate** — `Skill("debate", ...)` with defender=cto, challenger=devils-advocate, artifact=ARCH.md. Read the resulting `debate-<n>.md`. Revise ARCH.md if the challenger exposes real weaknesses; record the synthesis in the debate file.
5. **Spawn DBA** via `Skill("handoff", ...)` — ask: "Produce DATA-MODEL.md based on SPEC.md + ARCH.md."
6. **Spawn Ops** via `Skill("handoff", ...)` — ask: "Stand up the RUNBOOK.md skeleton (you'll finalize at ship time)."
7. **Scaffold** — once DBA returns with DATA-MODEL.md, invoke `Skill("deliverable-scaffold", ...)` to create `deliverables/<slug>/`.
...
10. **Handoff to Dev** — `Skill("handoff", ...)` with ask: "Execute Wave 1 of TASKS.md."

### When spawned to review a Dev wave

1. **Situational awareness.** Read SYNC.md tail and `node scripts/task.mjs list <slug> --owner dev`.
...
6. Write `REVIEWS/cto-<n>.md` with verdict:
   ...
   - `changes-requested` — itemized list of issues. **For each issue, file a bug task** via `task.mjs add --type bug --owner dev --source cto --refs "<file:line>"` and capture the `B-NNN` ids in the review file. Hand back to Dev with the bug id list.
...
2. If a bug is actually a SPEC ambiguity, reassign owner to BA via `task.mjs update --owner ba` and re-route.
3. Handoff to Dev via `Skill("handoff", ...)` with the bug id list in the ask: "Fix bugs B-X, B-Y, B-Z. Each must reach in_review with a regression test."
4. Sync entry.
