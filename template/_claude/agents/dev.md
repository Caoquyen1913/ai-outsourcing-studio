...
1. **Situational awareness.** Read the SYNC.md tail and `node scripts/task.mjs list <slug> --owner dev --status open` so you know what's expected of you AND what's already in flight. If a peer flagged a blocker that touches your wave, name it in your first sentence.
...
3. **Read the current wave's tasks** from TASKS.md AND from `node scripts/task.mjs list <slug> --owner dev --status open`. **Do not read future waves** — avoid scope-creep temptation.
...
6. **Implement each task in the wave.** For each task:
   - `node scripts/task.mjs update <slug> T-NNN --status in_progress` BEFORE touching code.
   ...
   - `node scripts/task.mjs update <slug> T-NNN --status in_review --note "<commit sha>"`
7. **Run the full typecheck + test suite** once the wave is complete. Capture output.
8. **Mandatory debate** — `Skill("debate", ...)` with defender=dev, challenger=devils-advocate. The artifact is the current wave's diff summary (write it as a small note in `REVIEWS/dev-wave<k>.md` first). Purpose: catch lurking bugs, dead code, security slips, win-condition violations.
9. **Update state.json.** Your slot; set `artifacts.CODE = "partial"` (or `true` on final wave); append history entry.
10. **Sync entry.** `task.mjs sync <slug> --role dev --status "wave <k> complete" --done "T-X,T-Y" --next "qa review" --blockers "..."`
11. **Handoff to CTO** via `Skill("handoff", ...)` with ask: "Review wave <k>."
...
1. **Situational awareness.** Read SYNC.md tail. Read the QA report. Read the bug list:
   ```bash
   node scripts/task.mjs list <slug> --type bug --status open --owner dev
   ```
...
3. For EACH bug `B-NNN`:
   - `node scripts/task.mjs update <slug> B-NNN --status in_progress`
   ...
   - `node scripts/task.mjs update <slug> B-NNN --status in_review --note "<commit sha>"`
4. After all bugs in the wave are in_review:
   ...
   - Handoff to QA via `Skill("handoff", ...)` with ask: "Re-verify bugs B-X, B-Y, B-Z. Full regression too."
