...
2. Verify pre-conditions:
   ...
   - **`node "$HOME/.claude/ai-outsourcing-studio/scripts/task.mjs" list <slug> --type bug --status open` returns zero rows**
   - **`node "$HOME/.claude/ai-outsourcing-studio/scripts/task.mjs" list <slug> --type bug --status in_review` returns zero rows**
