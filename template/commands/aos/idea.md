...
3. If no active project, spawn the **aos-ceo** subagent via the Task tool with this prompt (verbatim — do NOT soften the autonomy clause):
   ...
   > - When BRIEF.md is ready (with `## Win conditions`, `## Assumptions`, `## Delegated decisions`, zero `## Open questions`), invoke `Skill("aos-handoff", ...)` to BA **immediately**. Do NOT pause to confirm BRIEF with the user.
