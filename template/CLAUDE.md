...
Full system prompts live in `.claude/agents/<role>.md`.
...
**Debate checkpoints are mandatory.** Post-SPEC, post-DESIGN, post-ARCH, post-DATA-MODEL, and after each Dev wave, the owning role MUST invoke `Skill("debate", ...)` with the Devil's Advocate before handing off. This is enforced by role system prompts, not by code — so the rule appears in every relevant agent file.
...
**CLI** — every role uses `node scripts/task.mjs` to interact:

```bash
# Add a task (returns the new id on stdout)
node scripts/task.mjs add <slug> --type <feature|bug|chore|review|decision|risk> \
...
```
QA finds defect
  └─ QA: node scripts/task.mjs add <slug> --type bug --owner dev --source qa \
  ...
  └─ Dev: for each bug
       - read the bug (node scripts/task.mjs list <slug> --type bug --status open)
...
5. **Debate obligation** — At checkpoints, invoke `Skill("debate", ...)` with Devil's Advocate before handing off.
...
- `node scripts/update-board.mjs` — regenerate BOARD.md manually (normally hook-driven).
- `node scripts/log-chat.mjs` — hook entry point; not run manually.
