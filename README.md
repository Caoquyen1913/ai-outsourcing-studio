...
3. Pitch an idea:
   ```
   /idea "a tiny todo list web app with Google auth"
   ```
...
| Command    | Purpose |
|------------|---------|
| `/idea`    | Submit a new idea to the CEO |
| `/board`   | Print the live company board |
| `/standup` | Ask every role for a one-line status |
| `/kickoff` | Re-run CEO discovery on the active project |
| `/ship`    | Trigger final Ops + QA + CEO sign-off |
